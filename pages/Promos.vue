...
export default {
    ...
    computed: {
        ...mapState({
            blockData: state => state.content.contentData.brand.promos.blocks,
            downloadData: state => state.content.contentData.brand.promos.dlPageMaterials
        }),
        pageTitle() {
            return this.$store.state.lang === 'ru' ? 'Спортлайф | Рекламные материалы' : 'Спортлайф | Рекламные материалы'
        }
    },
    mounted() {
