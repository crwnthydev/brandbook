...
export default {
  ...
  methods: {
	...
	computed: {
		...mapState({
            blockData: state => state.content.contentData.brand.clothes.blocks,
            downloadData: state => state.content.contentData.brand.clothes.dlPageMaterials
		}),
		pageTitle() {
			return this.$store.state.lang === 'ru' ? 'Спортлайф | Одежда' : 'Спортлайф | Одежда'
		}
	},
	mounted() {
